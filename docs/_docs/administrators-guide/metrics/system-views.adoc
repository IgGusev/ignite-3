...
| SCHEMA | STRING | The schema used by the table.
| TABLE_NAME | STRING | Table name.
| TABLE_ID | INT32 | Unique table identifier.
| COLUMN_NAME | STRING | Column name.
| TYPE | STRING | Column data type.
| NULLABLE | BOOLEAN | If the column can be `NULL`.
| PREC | INT32 | Value precision. 0 if not applicable to data type.
| SCALE | INT32 | Value scale. 0 if not applicable to data type.
| LENGTH | INT32 | Value length, in bytes.
