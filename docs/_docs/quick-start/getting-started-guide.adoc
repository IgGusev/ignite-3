...
To initialize the cluster, you select the _metastorage group_ nodes - the group of nodes that will be responsible for managing the cluster, and provide a Ignite license.
...
tab:Command[]
----
cluster init --name=sampleCluster --metastorage-group=node1
----
...
* The `--metastorage-group` parameter specifies the nodes that will be used to store cluster meta information. In most scenarios, you want to have 3, 5 or 7 metastorage group nodes.  For more information on what they are and cluster lifecycle, see link:administrators-guide/lifecycle[Cluster Lifecycle].
+
...
tab:Command[]
----
cluster init --name=sampleCluster --metastorage-group=node1,node2,node3
----
